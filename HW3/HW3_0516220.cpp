...
void init(){
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(VertexAttribute) * earth_slices*(earth_stacks+1)*2, vertices, GL_STATIC_DRAW);
...
void display(){
	...
	glDrawArrays(GL_TRIANGLE_STRIP, 0, earth_slices*(earth_stacks+1)*2);
...
VertexAttribute *drawSphere(GLfloat radius, GLint slices, GLint stacks){
	VertexAttribute *vertices;
	vertices = new VertexAttribute[slices*(stacks+1)*2];
	float phi, theta;
	int cnt=0;
